...
from patent_hunter.fetchers import patentsview
...
async def fetch_node(
    state: PatentHunterState, *, runtime: GraphRuntime | None = None
) -> PatentHunterState:
    """Fetch candidate patents, using injected fixtures for dryrun/tests.

    The P1 module names the existing candidate fetcher ``fetch_patents`` rather
    than ``fetch_candidates``. If a future alias appears, this node will use it;
    otherwise it calls the current existing function directly.
    """

    runtime = runtime or GraphRuntime()
    if runtime.fetched_patents is not None:
        patents = list(runtime.fetched_patents)
    else:
        week = parse_iso_week(state["week"])
        cfg = patentsview.FetchConfig(
            vintage_years=runtime.vintage_years,
            max_per_category=runtime.max_per_category,
        )
        fetch_candidates = getattr(
            patentsview, "fetch_candidates", patentsview.fetch_patents
        )
        patents = await asyncio.to_thread(fetch_candidates, week, cfg)
...
async def score_sonnet_node(
    ...
    out = await scorer_sonnet.score_batch(
        state.get("fetched_patents", []), client=runtime.sonnet_client
    )
    return {
...
async def score_codex_node(
    ...
    out = await scorer_codex.score_batch(
        state.get("fetched_patents", []), runner=runtime.codex_runner
    )
    return {
...
def verify_node(
    ...
    for patent in state.get("fetched_patents", []):
        ...
        adopted = (
            ...
            and sonnet.score >= runtime.score_threshold
            and codex.score >= runtime.score_threshold
        )
        scored.append(
            ScoredPatent(
                patent=patent,
